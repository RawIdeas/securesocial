...
public abstract class OAuth2Provider extends IdentityProvider {
	...
	private Map<String, OAuth2> oauthMap;
	...
	protected OAuth2Provider(ProviderType type) {
		...
		key = getPropertiesKey(type);
		oauthMap = new HashMap<String, OAuth2>();
		scopeMap = new HashMap<String, String[]>();
	}

	/**
	 * Creates/Gets an OAuth2 object using the properties specified for the
	 * provider.
	 * 
	 * @param key
	 *            The provider key
	 ...
	 */
	private OAuth2 oAuth2(String key, String domain) {

		// Making a compound key from the existing key and domain
		String compoundKey = new StringBuilder(key).append(domain).append(".").toString();

		// if the exists in already exists table return the current one
		if (oauthMap.containsKey(compoundKey)) {
			return oauthMap.get(compoundKey);
		}

		// otherwise create a new OAuth and put it in the hashtable
		OAuth2 newOAuth = new OAuth2(Play.configuration.getProperty(compoundKey + AUTHORIZATION_URL),
				Play.configuration.getProperty(compoundKey + ACCESS_TOKEN_URL),
				Play.configuration.getProperty(compoundKey + CLIENTID), Play.configuration.getProperty(compoundKey
						+ SECRET));

		oauthMap.put(compoundKey, newOAuth);

		return newOAuth;
	}

	private String[] scope(String key, String domain) {
		// Making a compound key from the existing key and domain
		String compoundKey = new StringBuilder(key).append(domain).append(".").toString();

		if (scopeMap.containsKey(compoundKey)) {
			return scopeMap.get(compoundKey);
		}

		String[] newScope = getScope(compoundKey, SCOPE);

		scopeMap.put(compoundKey, newScope);

		return newScope;
	}

	/**
	 * Returns the scope specified for this provider in the properties file (eg:
	 * facebook.scope)
	 * 
	 * @param providerKey
	 *            The properties key
	...
	protected SocialUser doAuth(Map<String, Object> authContext) {
		...
		if (params.get(ERROR) != null) {
			// todo: improve this. Get details of the error and include them in
			// the exception.
			throw new AuthenticationException();
		}
		String domain = (String) authContext.get(DOMAIN_CONTEXT);
		OAuth2 service = oAuth2(key, domain);
		...
		if (response.error != null) {
			if (response.error.type == OAuth2.Error.Type.UNKNOWN) {
				// the OAuth2 class is expecting the access token in the query
				// string.
				// this is not what the OAuth2 spec says. Facebook works, but
				// Foursquare fails for example.
				// So I'm going to check if the token is there before throwing
				// the exception.
				...
				if (asJson != null) {
					...
					if (body != null) {
						// this is what many libraries expect (probably because
						// Facebook returns it)
						...
						if (token != null) {
							...
							// this is what should be returned as defined in the
							// OAuth2 spec
			...
			// if the workaround did not find the token then we really have an
			// error,
